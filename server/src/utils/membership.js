...
const PLAN_LIMITS = Object.freeze({ basic: 3, premium: 50, extreme: null });
...
const normalizeMembership = (account) => {
  ...
  const plan = activePaid && ['premium', 'extreme'].includes(membership.plan) ? membership.plan : 'basic';
  const today = dayKey();
  ...
  return {
    ...
    price: plan === 'premium' ? 5 : plan === 'extreme' ? 12 : 0,
    currency: 'GBP',
    aiMessagesUsed: used,
...
const consumeAiMessage = async (account) => {
  ...
  } else if (entitlement.plan === 'premium') {
    const today = dayKey();
