...
const CookieStartPage = ({ onChoice }) => (
  <main className="cookie-start-page" aria-label="Cookie notice">
    <section className="cookie-start-card">
      <svg className="cookie-start-svg" x="0px" y="0px" viewBox="0 0 122.88 122.25" xmlSpace="preserve" aria-hidden="true">
        <g>
      ...
      <p className="cookie-start-heading">We use cookies.</p>
      <p className="cookie-start-description">
        This website uses cookies to ensure you get the best experience on our site.
      </p>
      <div className="cookie-start-actions">
        <button type="button" className="cookie-start-accept" onClick={() => onChoice('allowed')}>
          Allow
        </button>
        <button type="button" className="cookie-start-decline" onClick={() => onChoice('declined')}>
          Decline
        </button>
      </div>
    </section>
...
function App() {
  ...
  const handleCookieChoice = (choice) => {
    try {
      ...
      // Continue into the app even when storage is unavailable.
    }
    setCookieConsent(choice);
  };

  if (!cookieConsent) {
    return <CookieStartPage onChoice={handleCookieChoice} />;
  }

  return (
    <BrowserRouter>
      <AppRoutes appMode={appMode} setAppMode={setAppMode} />
    </BrowserRouter>
  );
}
